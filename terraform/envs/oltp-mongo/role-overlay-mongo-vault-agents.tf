...
locals {
  ...
  mongo_vault_agent_active = {
    ...
    if var.enable_mongo_vault_agents && spec.enabled
  }
...
resource "null_resource" "mongo_vault_agent" {
  ...
  triggers = {
    ...
    nftables_id        = length(null_resource.mongo_nftables_backplane) > 0 ? null_resource.mongo_nftables_backplane[0].id : "disabled"
  ...
  depends_on = [null_resource.mongo_nftables_backplane]
