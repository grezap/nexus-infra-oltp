...
output "next_step" {
  value = <<-EOT
    ...
    Apply order (per docs/handbook.md s1):
      ...
      2. nexus-infra-vmware: pwsh -File scripts/foundation.ps1 apply (dhcp reservations).
      3. nexus-infra-vmware: pwsh -File scripts/security.ps1   apply (PKI + AppRoles + sidecars).
