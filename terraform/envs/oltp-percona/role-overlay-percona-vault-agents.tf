/*
 * role-overlay-percona-vault-agents.tf -- Phase 0.G.3 (chunk 3a)
 *
 * Installs Vault Agent as a `nexus-vault-agent` systemd service on each of
 * the 5 percona-tier clones (3 PXC + 2 ProxySQL). Linear port of
 * role-overlay-mongo-vault-agents.tf with 5 hosts instead of 3 + percona-
 * specific sidecar prefix (`oltp-percona-`).
 *
 * Cross-env coupling: reads the per-host AppRole JSON sidecars at
 * $HOME/.nexus/vault-agent-oltp-percona-<host>.json (written by
 * nexus-infra-vmware/terraform/envs/security/role-overlay-vault-agent-
 * percona-approles.tf). ERROR (not WARN+skip) if absent.
 *
 ...
 * sink + vault address). role-overlay-percona-tls.tf (chunk 3b) drops the
 * PKI template stanza as 70-template-percona-tls.hcl + 4 KV template
 * stanzas (71-cluster-password.hcl, 72-monitor-password.hcl,
 * 73-root-password.hcl OR 73-proxysql-admin.hcl depending on role)
 * without rewriting this file.
 *
 * Selective ops: var.enable_percona_vault_agents (master) AND per-host
 *                var.enable_{pxc_node,proxysql}_<n>_vault_agent.
 *
...
locals {
  percona_vault_agent_specs = {
    "pxc-node-1" = { vm_ip = "192.168.70.51", enabled = var.enable_pxc_node_1_vault_agent }
    "pxc-node-2" = { vm_ip = "192.168.70.52", enabled = var.enable_pxc_node_2_vault_agent }
    "pxc-node-3" = { vm_ip = "192.168.70.53", enabled = var.enable_pxc_node_3_vault_agent }
    "proxysql-1" = { vm_ip = "192.168.70.54", enabled = var.enable_proxysql_1_vault_agent }
    "proxysql-2" = { vm_ip = "192.168.70.55", enabled = var.enable_proxysql_2_vault_agent }
  }

  percona_vault_agent_active = {
    for host, spec in local.percona_vault_agent_specs : host => spec
    if var.enable_percona_vault_agents && spec.enabled
  }

  # Terraform pathexpand() only handles `~`, NOT `$HOME`. Substitute first.
  percona_va_creds_dir_expanded = pathexpand(replace(var.vault_agent_percona_creds_dir, "$HOME", "~"))
  percona_va_ca_bundle_expanded = pathexpand(replace(var.vault_pki_ca_bundle_path, "$HOME", "~"))
}

resource "null_resource" "percona_vault_agent" {
  for_each = local.percona_vault_agent_active

  triggers = {
    creds_file_path      = "${local.percona_va_creds_dir_expanded}/vault-agent-oltp-percona-${each.key}.json"
    creds_file_hash      = filesha256("${local.percona_va_creds_dir_expanded}/vault-agent-oltp-percona-${each.key}.json")
    nftables_id          = length(null_resource.percona_nftables_backplane) > 0 ? null_resource.percona_nftables_backplane[0].id : "disabled"
    vault_version        = var.vault_agent_version
    percona_va_overlay_v = "1" # v1 (0.G.3) = initial 5 nodes (3 PXC + 2 ProxySQL).

    destroy_vm_ip    = each.value.vm_ip
    destroy_ssh_user = var.oltp_node_user
  }

  depends_on = [null_resource.percona_nftables_backplane]
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      $credsFile    = '${local.percona_va_creds_dir_expanded}/vault-agent-oltp-percona-${each.key}.json'
      $caBundlePath = '${local.percona_va_ca_bundle_expanded}'
      $sshUser      = '${var.oltp_node_user}'
      $bootTimeout  = ${var.oltp_cluster_timeout_minutes}

      if (-not (Test-Path $credsFile)) {
        throw "[percona-va $hostName] creds file $credsFile missing -- run nexus-infra-vmware/scripts/security.ps1 apply FIRST to provision the 5 percona AppRole sidecars."
      }
      ...
      if (-not $roleId -or -not $secretId) {
        throw "[percona-va $hostName] creds JSON missing role_id or secret_id"
      }
      if (-not (Test-Path $caBundlePath)) {
        throw "[percona-va $hostName] CA bundle $caBundlePath missing -- run security env apply (PKI distribute) first."
      }

      $sshOpts = @('-o','ConnectTimeout=10','-o','BatchMode=yes','-o','StrictHostKeyChecking=no')

      Write-Host "[percona-va $hostName] waiting for SSH + firstboot marker..."
      ...
      if (-not $booted) { throw "[percona-va $hostName] SSH + firstboot marker never ready after $bootTimeout min" }
      ...
      if ($probe -match "Vault v$vaultVersion" -and $probe -match '(?m)^active$') {
        Write-Host "[percona-va $hostName] already installed at v$vaultVersion + service active; skipping."
        exit 0
      }

      Write-Host "[percona-va $hostName] installing Vault Agent v$vaultVersion"

      $installScript = @"
set -euo pipefail
if ! getent hosts releases.hashicorp.com >/dev/null 2>&1; then
  echo "[percona-va install] /etc/resolv.conf missing resolver; pointing at nexus-gateway dnsmasq"
...
"@
      ...
      if ($LASTEXITCODE -ne 0) {
        ...
        throw "[percona-va $hostName] vault binary install failed (rc=$LASTEXITCODE)"
      }
      Write-Host $installOut.Trim()
...
"@
        ...
        if ($LASTEXITCODE -ne 0) {
          ...
          throw "[percona-va $hostName] credential staging failed (rc=$LASTEXITCODE)"
        }
      } finally {
      ...
      $baseConfig = @"
# 00-base.hcl -- Phase 0.G.3. auto_auth (approle) + sink + vault address.
# role-overlay-percona-tls.tf (chunk 3b) drops 70-template-percona-tls.hcl
# + KV template stanzas (cluster-password, monitor-password, root-password
# for PXC OR proxysql-admin-password for ProxySQL) in this dir to add the
# PKI cert + secret render templates without rewriting this file.
...
"@
      ...
      $unitFile = @"
[Unit]
Description=Nexus Vault Agent (Phase 0.G.3 -- Percona XtraDB + ProxySQL mTLS + cluster creds)
...
"@
      ...
      if ($LASTEXITCODE -ne 0) {
        ...
        throw "[percona-va $hostName] config/service setup failed (rc=$LASTEXITCODE)"
      }
      Write-Host $finalOut.Trim()
      ...
      if (-not $serviceActive) {
        ...
        Write-Host $journal
        throw "[percona-va $hostName] nexus-vault-agent.service failed to reach active within 30s"
      }
      Write-Host "[percona-va $hostName] nexus-vault-agent.service active"
      ...
      if ($tokenCheck -notmatch 'TOKEN_PRESENT') {
        $journal = (ssh @sshOpts "$sshUser@$vmIp" "sudo journalctl -u nexus-vault-agent.service --no-pager -n 30" 2>&1 | Out-String)
        Write-Host $journal
        throw "[percona-va $hostName] AppRole login appears to have failed (token sink empty)"
      }
      Write-Host "[percona-va $hostName] AppRole authenticated; token sink populated"
    PWSH
  }

  # Surgical destroy: removes install-owned files only. The TLS template +
  # KV templates (dropped by role-overlay-percona-tls.tf) survive an
  ...
  provisioner "local-exec" {
    ...
    command     = <<-PWSH
      ...
      Write-Host "[percona-va destroy] $${hostName}: stopping nexus-vault-agent + cleaning install-owned files (keeping /etc/vault-agent/ + TLS/KV templates)"
