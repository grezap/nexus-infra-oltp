...
locals {
  ...
  redis_vault_agent_active = {
    ...
    if var.enable_redis_vault_agents && spec.enabled
  }
...
resource "null_resource" "redis_vault_agent" {
  ...
  triggers = {
    ...
    # install the agent.
    nftables_id        = length(null_resource.redis_nftables_backplane) > 0 ? null_resource.redis_nftables_backplane[0].id : "disabled"
  ...
  depends_on = [null_resource.redis_nftables_backplane]
