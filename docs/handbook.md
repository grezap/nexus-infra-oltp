...
| 27 | After #26 fixes (iSCSI LUN attached on both FCI nodes, S:\ formatted), apply v14 cleared iscsi-attach but `wsfc_bootstrap` failed: `Test-Cluster : Access is denied.` then (after skipping Test-Cluster) `New-Cluster : ... node cannot be contacted`. | WSFC cmdlets (Test-Cluster, New-Cluster, Add-ClusterDisk) need DOMAIN admin + cross-node RPC. The Windows OpenSSH SSH session runs as LOCAL `sql-fci-1\nexusadmin` (key auth keeps the local SAM identity even on a domain-joined host) -- no Kerberos TGT, not a cluster admin. | Dispatch the WSFC orchestrate script as a Scheduled Task running as `NEXUS\nexusadmin` (creds from nexusadmin-credentials.json sidecar) in role-overlay-wsfc-bootstrap.tf. Sub-transients: **#27b** -- `Register-ScheduledTask -Principal + -Password` = AmbiguousParameterSet; **#27c** -- `Register-ScheduledTask -User -Password` defaults to S4U logon type (constrained token, NO network creds) → cross-node RPC still fails; fix is `schtasks.exe /Create /RU /RP` (legacy syntax = Password logon type, full network creds); skip Test-Cluster preflight + use `New-Cluster -Force`. |
| 27d | After #27c (schtasks Password logon), New-Cluster still failed "node cannot be contacted" -- but now from a valid domain context. `Resolve-DnsName sql-ag-rep-1` returned `sql-ag-rep-1.nexus.local` (WRONG suffix; AD domain is nexus.lab). `Get-DnsClient` showed Ethernet0 ConnectionSpecificSuffix = `nexus.local`. | nexus-gateway's dnsmasq.conf had `dhcp-option=option:domain-name,nexus.local` + `domain=nexus.local` (legacy from the pre-AD lab era when the domain was nexus.local). The Windows DHCP client picks up DHCP option 15 → sets the per-NIC connection suffix → registers + resolves cluster nodes under nexus.local, which AD/Kerberos doesn't serve → cross-node cluster contact fails. | Fix dnsmasq.conf on nexus-gateway: `sed` `nexus.local` → `nexus.lab` in both the `dhcp-option=option:domain-name` + `domain=` lines, `systemctl restart dnsmasq`, then `ipconfig /release + /renew + /registerdns` on all 4 SQL nodes to pick up the corrected suffix. Permanent fix belongs in nexus-infra-vmware foundation env's gateway dnsmasq overlay (the domain option should have been nexus.lab from 0.D.3 AD bring-up). After this, WSFC New-Cluster succeeded: cluster `sql-fci-cluster` live at .70.15 with all 4 nodes Up + cluster disk added. |
| 28 | During the FCI-install ISO upload (1.26 GB scp'd to sql-fci-1 + sql-fci-2 in PARALLEL), all 4 SQL VMs powered off uncleanly (`vmrun list` showed only 6 of 10 VMs; the 4 SQL nodes gone; vmware.log: `cleanShutdown = "FALSE"`). The build host had 218 GB free RAM + 4.8 TB free disk, so not a resource-exhaustion -- likely a transient VMware Workstation host I/O stall under the simultaneous large writes. | Parallel multi-GB scp into multiple running VMs on the same VMware host can stall the host's storage stack enough to wedge the guests. Per memory `feedback_vmrun_unknown_error_transient.md` (vmrun sporadic failures), VMware Workstation is not bulletproof under I/O load. The unclean power-off ALSO truncated both in-flight ISO writes to 0 bytes (the file existed but the data was still in the write cache, lost on power-off). | Recovery: (a) `vmrun start` all 4 SQL VMs (firstboot AtStartup re-runs idempotently; WSFC cluster + domain membership + S:\ all survived since they're persisted to disk + AD). (b) Re-upload the ISOs SEQUENTIALLY (one node at a time) instead of parallel, with post-upload size verification (`(Get-Item ...).Length` must equal the 1265688576-byte source). Lesson: large file pushes to lab VMs should be sequential + size-verified; never trust scp rc=0 alone across an unclean-shutdown window. |
