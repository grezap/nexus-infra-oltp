...
| Apply fails at `[redis-tls redis-N] cert files not rendered ... within 60s` | Vault Agent template syntax error (rare; per-host CN/SAN substitution), OR the PKI role's `allowed_domains` doesn't cover the requested CN. | `ssh nexusadmin@<ip>; sudo journalctl -u nexus-vault-agent.service -n 50`. Check for `template syntax error` or `403 access denied`. Re-apply security env's `vault_pki_redis_role` if `allowed_domains` was outdated. |
| Apply fails at `[redis-config redis-N] nexus-redis.service / TLS PING did not converge within 20 min` | nexus-redis.service crashlooping. Most likely: TLS material missing or PKCS#1 vs PKCS#8 mismatch. | `ssh nexusadmin@<ip>; sudo journalctl -u nexus-redis.service -n 50`. Check that `/etc/nexus-redis/tls/server.key` starts with `-----BEGIN PRIVATE KEY-----` (PKCS#8), NOT `-----BEGIN RSA PRIVATE KEY-----` (PKCS#1). Re-run `oltp.ps1 apply -- -target='null_resource.redis_tls["redis-N"]'`. |
