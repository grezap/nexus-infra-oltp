...
| 8 | `packer build oltp-sqlserver-node` v2 (after #7 fix) progressed past clone + power-on but errored after 30:30 with `Timeout waiting for SSH.` -- the cloned VM was running but unreachable. | The source `ws2025-desktop.vmx` was baked with `vmx_remove_ethernet_interfaces = true` so the artifact has ZERO NIC entries. The `vmware-vmx` builder clones the .vmx as-is + doesn't auto-add NICs the way `vmware-iso` does (vmware-iso's `network` + `network_adapter_type` params inject NICs at clone time; vmware-vmx's same params are documented but in practice only override existing NIC entries). Result: cloned VM came up + OOBE'd + nexusadmin's OpenSSH service started, but with no NIC there's no IP -> Packer's IP-discovery-via-VMware-Tools returned empty -> SSH probe timed out. Same architectural class as the terraform/modules/vm canon (which uses `vmrun clone` + `configure-vm-nic.ps1` to add NICs post-clone) -- but Packer's vmware-vmx has no equivalent NIC-injection hook. | Add NIC entries via `vmx_data`: `ethernet0.present=TRUE`, `connectionType=nat`, `virtualDev=e1000e`, `addressType=generated`, `startConnected=TRUE`. The build-time NAT NIC lets the cloned VM reach DHCP + Packer find it via VMware Tools' IP report. Also added `vmx_remove_ethernet_interfaces = true` at the OUTPUT side so the baked oltp-sqlserver-node.vmx is itself NIC-less -- which is what terraform/modules/vm + configure-vm-nic.ps1 expect (the canon pattern from 0.B.5 onwards). Permanent fix in `oltp-sqlserver-node.pkr.hcl` source block. |
