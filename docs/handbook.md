...
| 15 | After #14 fix setup.exe progressed PAST the CLI-validation gate (the new args were accepted) + into FinalCalculateSettings, but failed there with the same `0x84B40001` catch-all wrapping a nested `System.Security.Cryptography.CryptographicException @ -2147024891`. The enhanced error capture (#14) surfaced this from Detail.txt -- without it we'd still be flying blind. | `-2147024891` = `0x80070005` = `E_ACCESSDENIED`. SQL Setup's FinalCalculateSettings step encrypts the SA password into the ConfigurationFile.ini using a CNG (Cryptography Next Generation) provider. SQL Server 2025 changed the default CNG provider for password encryption to one that may attempt TPM-backed key storage. VMware Workstation Pro 25 does not expose a vTPM to the guest at bake time (the Autounattend comment in `_shared/powershell/floppy/Autounattend.xml.tpl` documents this: "standalone Workstation cannot expose a real vTPM via Packer"). So the crypto op fails with access denied + Setup wraps it as the generic XML-document error. | Drop `/SECURITYMODE=SQL` + `/SAPWD=...` from the bake-time setup.exe args -- install with Windows auth only. The SA login can still exist (Windows-auth doesn't preclude SQL-auth logins from being added later); terraform's `role-overlay-fci-install.tf` enables SQL auth + sets SA password via T-SQL using the KV-seeded value at apply time. Permanent fix in `10-sql-install.ps1` setupArgs. |
