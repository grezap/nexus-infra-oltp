/*
 * oltp-mongo-node -- MongoDB Replica Set node template (Phase 0.G.3.5+).
 * Per-engine refactor per memory/feedback_per_cluster_state_per_engine_template.md.
 * 3 clones: mongo-1/2/3 (.71/.72/.73). ~2 GB template. Replaces mongo portion
 * of the legacy monolithic oltp-node.vmx.
 *
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      # apt-shipped mongod.service MUST be disabled + masked so a cold boot
      # does not race nexus-mongo.service for port 27017.
