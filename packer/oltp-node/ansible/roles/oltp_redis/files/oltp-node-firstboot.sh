...
# Canon: nexus-platform-plan/docs/infra/vms.yaml (cluster: redis lines
# 182-191 for 0.G.1; mongo/percona/patroni extend per sub-phase).
...
case "$VMNET11_IP" in
  ...
  # ─── 0.G.2 -- MongoDB replica set (3 nodes) -- TODO when sub-phase ships ─
  ...
  *)
    ...
    echo "$LOG_PREFIX recognised IPs: redis-1..6 (.81/.82/.83/.84/.87/.89); other 0.G.* clusters land later sub-phases." >&2
    exit 1
    ;;
esac
...
# Path under /etc/nexus-redis/ for 0.G.1 (the Redis cluster's dir). Later
# sub-phases' role-overlays MAY relocate or symlink this -- the identity
# data is per-tier, not per-cluster.
...
EOF
chown root:redis "$IDENTITY_FILE"
