/*
 * oltp-redis-node -- Packer template variables (Phase 0.G.3.5+)
 *
 * Per-engine template per the 0.G.3.5 refactor. Strips mongo/pxc/proxysql
 * vars (each ships in its own sibling template).
 */

variable "vm_name" {
  type        = string
  default     = "oltp-redis-node"
  description = "VM display name and output .vmx basename. Default `oltp-redis-node` -- the template; per-clone names (redis-1..6) are set by terraform/envs/oltp-redis/."
}

variable "output_directory" {
  type        = string
  default     = "H:/VMS/NexusPlatform/_templates/oltp-redis-node"
