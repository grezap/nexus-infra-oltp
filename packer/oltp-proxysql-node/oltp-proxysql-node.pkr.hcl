/*
 * oltp-proxysql-node -- ProxySQL + keepalived node template (Phase 0.G.3.5+).
 * Per-engine refactor per memory/feedback_per_cluster_state_per_engine_template.md.
 * 2 clones: proxysql-1/2 (.54/.55) + VRRP-floated VIP .50. ~2 GB template.
 * Replaces proxysql portion of the legacy monolithic oltp-node.vmx.
 * Bakes ProxySQL 2.6.x from vendor flat-repo + keepalived from Debian main.
 *
...
source "vmware-iso" "oltp-node" {
  ...
  vmx_data = {
    "annotation"           = "oltp-proxysql-node template (Phase 0.G.3.5+) -- built by Packer; ProxySQL ${var.proxysql_version} (vendor flat-repo) + keepalived (Debian apt main)"
...
build {
  ...
  provisioner "shell" {
    inline = [
      "echo '--- oltp-proxysql-node post-install checks ---'",
      ...
      # apt-shipped proxysql.service MUST be masked so cold boot doesn't
      # auto-load a default config + race nexus-proxysql.service for :6033.
      "systemctl is-enabled proxysql.service 2>&1 | grep -qE '^(masked|disabled)$' || (echo 'ERROR: proxysql.service is not masked/disabled' && exit 1)",
      "id proxysql",
      "echo '--- cleanup ---'",
      "sudo apt-get clean",
