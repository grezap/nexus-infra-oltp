/*
 * oltp-etcd-node -- Packer template variables (Phase 0.G.4)
 */

variable "vm_name" {
  type        = string
  default     = "oltp-etcd-node"
  description = "VM display name and output .vmx basename."
}

variable "output_directory" {
  type        = string
  default     = "H:/VMS/NexusPlatform/_templates/oltp-etcd-node"
...
variable "etcd_version" {
  type        = string
  default     = "3.5.16"
  description = "etcd version downloaded from upstream GitHub releases (https://github.com/etcd-io/etcd/releases/download/v{{ version }}/etcd-v{{ version }}-linux-amd64.tar.gz). Default 3.5.16 (current stable patch on the 3.5 LTS line). Patroni 4.x's etcd3 client expects the v3 gRPC API which is stable since 3.5.0; pinning to 3.5.16 gets the latest security fixes within the LTS series."
}
