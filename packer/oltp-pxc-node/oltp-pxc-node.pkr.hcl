/*
 * oltp-pxc-node -- Percona XtraDB Cluster node template (Phase 0.G.3.5+).
 * Per-engine refactor per memory/feedback_per_cluster_state_per_engine_template.md.
 * 3 clones: pxc-node-1/2/3 (.51/.52/.53). ~2 GB template. Replaces pxc portion
 * of the legacy monolithic oltp-node.vmx. The oltp_pxc role baked here carries
 * ALL 12 PXC-specific transient fixes from 0.G.3 ratification (manual apt
 * source, GPG extract from .deb, libaio1 bookworm fallback, AppArmor disable,
 * mysqld --initialize-insecure, /usr/local/sbin/nexus-pxc-mysql wrapper).
 *
 * LEGACY (preserved for context until oltp-node.vmx is retired in 0.G.3.5c):
...
source "vmware-iso" "oltp-node" {
  ...
  vmx_data = {
    "annotation"           = "oltp-pxc-node template (Phase 0.G.3.5+) -- built by Packer; Percona XtraDB Cluster ${var.pxc_version} (Percona vendor apt, manual source pinned to bookworm)"
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      # apt-shipped mysql.service + mysql@bootstrap.service MUST be masked
      # to prevent auto-bootstrap of a junk cluster on first boot.
      "systemctl is-enabled mysql.service           2>&1 | grep -qE '^(masked|disabled)$' || (echo 'ERROR: mysql.service is not masked/disabled (Percona apt-shipped; would auto-bootstrap)' && exit 1)",
      "systemctl is-enabled mysql@bootstrap.service 2>&1 | grep -qE '^(masked|disabled|static)$' || (echo 'ERROR: mysql@bootstrap.service is not masked/disabled/static' && exit 1)",
      # AppArmor mysqld profile MUST be disabled (0.G.3 transient #11) --
      # default profile only allows /var/lib/mysql writes; our datadir is
      # /var/lib/nexus-percona.
      "test -L /etc/apparmor.d/disable/usr.sbin.mysqld || (echo 'ERROR: AppArmor mysqld profile not disabled' && exit 1)",
      # Datadir MUST be initialized (0.G.3 transient #8) -- mysqld won't
      # start without the mysql.* system tables.
      "sudo test -f /var/lib/nexus-percona/mysql/general_log.CSM || (echo 'ERROR: Percona datadir not initialized -- run mysqld --initialize-insecure' && exit 1)",
      "id mysql",
      "echo '--- cleanup ---'",
      "sudo apt-get clean",
